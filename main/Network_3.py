...
def down_block(input_layer, filters, kernel_size=(3,3), activation="relu"):
    ...
    # print(output.shape)
    output = Conv2D(filters, kernel_size, padding="same", activation=activation, data_format='channels_last')(output)
    # print(output.shape)
    output = Conv2D(filters*2, kernel_size, padding="same", activation=activation, data_format='channels_last')(output)
    # print(output.shape)
    # print('-------------')
...
def up_block(input_layer, residual_layer, filters, kernel_size=(3,3),activation="relu"):
    ...
    output = Conv2DTranspose(filters, kernel_size, activation='relu', padding='same', data_format='channels_last')(output)
    output = Conv2DTranspose(filters, kernel_size, activation='relu', padding='same', data_format='channels_last')(output)
    output = Conv2DTranspose(filters*2, kernel_size, activation='relu', padding='same', data_format='channels_last')(output)
    return output
...
def build_mbllen(input_shape):
    ...
    def EM(input, kernel_size, channel):
        ...
        down2, pool2 = down_block(pool1, channel*8,(kernel_size, kernel_size))
        # print(down2.shape, pool2.shape)

        down3 = Conv2D(channel*32, kernel_size=(3,3), padding="same", activation="relu", data_format='channels_last')(pool2)
        # print(down3.shape)
        down4 = Conv2D(channel*64, kernel_size=(3,3), padding="same", activation="relu", data_format='channels_last')(down3)
        # print(down4.shape)

        up = up_block(down4, down2, channel*4, (kernel_size, kernel_size))#giving half channels since *2 in upblock
        up = up_block(up, down1, channel, (kernel_size, kernel_size))
        # print(up.shape)
        res = Conv2DTranspose(3, (kernel_size,kernel_size), activation='relu', padding='same', data_format='channels_last')(up)
